...
function EventTarget() {
  this._jsdomEvents = Object.create(null);
}
...
EventTarget.prototype.addEventListener = function (type, callback, capture) {
  ...
  if (this._jsdomEvents[type] === undefined) {
    this._jsdomEvents[type] = {
      capturing: [],
      bubbling: []
    };
  }

  var handlers = this._jsdomEvents[type][capture ? "capturing" : "bubbling"];
...
function getListeners(target, type, capturing) {
  var listeners;
  if (target._jsdomEvents[type]) {
    listeners = target._jsdomEvents[type][capturing ? "capturing" : "bubbling"];
  } else {
    listeners = [];
  }
